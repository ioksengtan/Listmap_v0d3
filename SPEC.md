...
4. **Landmarks belonging to that story derive their ids from the same issue number `N` — never from "current max in `data/landmarks.csv` + 1".** Compute `landmark_block_start = 200000 + N * 50`; that story's landmarks get ids `landmark_block_start` through `landmark_block_start + 49` (50 slots — use as many or as few as the story needs, low-to-high, and leave the rest unused). Because `N` is already unique (same atomic counter as `story_id`), every story's 50-slot block is automatically disjoint from every other story's block, with zero coordination and without ever reading what any other session has already written. Do this even for a solo, uncontested-feeling session — the whole point is that no one has to check.

   *(This replaces an earlier version of this rule that told landmarks to keep incrementing within the shared 100000+ lane, on the theory that a plain counter was "already disjoint." It wasn't: multiple concurrent sessions each computed their own "current max + 1" from the same stale CSV snapshot, repeatedly picked the same next id, and had to renumber and re-merge after the fact — it happened three times in one session on 2026-09-24 alone. The per-story block above removes the shared counter entirely, so there's nothing left to race on. Landmark ids already assigned under the old scheme — anything below 200000 — are left as-is; only new reservations use the 200000+ block scheme.)*
...
| Range | Reserved for |
|-------|--------------|
| below 5000 | existing convention (Cursor / manually authored) — continue incrementing from the current max as before, for both `story_id` and `landmark_id` |
| 100000+ | `story_id` under the issue-reservation scheme (`100000 + N`). Also the legacy home for `landmark_id`s assigned before the per-story block scheme existed (see below) — those are left as-is, not migrated. |
| 200000+ | `landmark_id` under the issue-reservation scheme — per-story blocks of 50, `200000 + N * 50` through `+ 49` (step 4 above). If no issue can be reserved (offline, no repo write access), fall back to flat-incrementing from the current max **within 200000+** rather than 100000+, and accept the same collision risk the old flat scheme had — this fallback is a last resort, not the default. |

The gap between the lanes is deliberately wide and left unused: at the growth rate observed in the first weeks of this convention (~7 stories / 4 days, ~4–6 landmarks per story), a flat-incrementing lane could plausibly climb ~8–10/day — a number with no headroom margin isn't a safe boundary. Each floor (5000, 100000, 200000) gives decades of runway at that pace with zero ongoing cost, since these are plain integers with no format constraint. If any lane ever does approach its ceiling, raising it further is a one-line edit here — it doesn't require renumbering anything already assigned, only shifts where the *next* id starts.

`scripts/check-unique-ids.js` remains the backstop for the rare case any of this still produces a collision.
